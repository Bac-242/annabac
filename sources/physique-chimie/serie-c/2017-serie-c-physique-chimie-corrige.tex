...
\entetecorrige{2017}{C}{Physique-Chimie}
...
\begin{methode}
Équation de désintégration : écrire la particule émise avec ses
nombres ($\beta^-$ : $^{0}_{-1}e$ ; $\alpha$ : $^{4}_{2}\text{He}$),
puis appliquer les deux lois de conservation — nombre de masse $A$ et
nombre de charge $Z$ — pour identifier le noyau fils dans la
...
\begin{methode}
Démarche type en dynamique : \textbf{1)} définir le système et le
référentiel (galiléen) ; \textbf{2)} inventaire des forces (schéma) ;
\textbf{3)} TCI projeté sur des axes adaptés pour la translation, RFD
($\sum \mathcal{M} = J\ddot{\theta}$) pour la rotation ;
\textbf{4)} relier les deux par le fil inextensible : $T = T'$ et
$a = r\ddot{\theta}$. La méthode énergétique, suivie d'une dérivation
en temps, doit redonner la même accélération — bon moyen de contrôle.
\end{methode}
...
\begin{rappel}
Effet photoélectrique : l'extraction n'a lieu que si l'énergie du
photon dépasse le travail d'extraction, $W = \frac{hc}{\lambda} > W_0$
(soit $\lambda < \lambda_0$). Le bilan d'énergie s'écrit alors
$W = W_0 + E_{c\max}$ : l'excédent part en énergie cinétique de
l'électron.
\end{rappel}
