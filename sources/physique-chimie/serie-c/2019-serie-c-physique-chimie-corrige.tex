...
\begin{methode}
Reconnaître la force d'un acide à partir du pH : calculer
$-\log C_0$. Si $\text{pH} = -\log C_0$, l'acide est totalement
dissocié : acide \emph{fort}. Si $\text{pH} > -\log C_0$, la
dissociation est partielle : acide \emph{faible}. (Raisonnement
symétrique pour une base avec $14 + \log C_0$.)
\end{methode}
...
\begin{piege}
Deux sinusoïdes qui se ressemblent, deux grandeurs différentes : la
courbe $y(t)$ (élongation d'un point au cours du temps) donne la
\emph{période} $T$, la courbe $y(x)$ (photo de la corde à un instant)
donne la \emph{longueur d'onde} $\lambda$. Vérifier l'axe des
abscisses avant toute lecture.
\end{piege}
\begin{center}
...
\begin{methode}
Théorème de l'énergie cinétique sur une piste courbe :
$\Delta E_c = \sum W$. La réaction normale ne travaille jamais
(perpendiculaire au déplacement) ; le frottement travaille sur la
\emph{longueur du chemin} ($r\theta$ pour un arc, pas la corde) ; le
poids sur la \emph{dénivellation} $h = r(1 - \cos\theta)$, lue par
