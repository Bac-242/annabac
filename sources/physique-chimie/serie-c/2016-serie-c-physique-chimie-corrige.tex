...
\begin{methode}
Inventaire des concentrations d'une solution : toujours le même
enchaînement — \textbf{1)} $[\ch{H_3O^+}] = 10^{-\text{pH}}$ ;
\textbf{2)} $[\ch{HO^-}] = K_e/[\ch{H_3O^+}]$ ;
\textbf{3)} \emph{électroneutralité} (somme des charges $+$ $=$ somme
des charges $-$), en négligeant l'espèce très minoritaire ;
\textbf{4)} \emph{conservation de la matière} pour la dernière espèce.
\end{methode}
...
\begin{rappel}
Solution tampon : mélange équimolaire d'un acide faible et de sa base
conjuguée, obtenu notamment à la \emph{demi-équivalence} d'un dosage —
où pH $=$ p$K_a$. Son pH varie peu par ajout modéré d'acide ou de base,
et il est insensible à une dilution modérée.
\end{rappel}
...
\begin{methode}
Onde progressive : un point $P$ situé à la distance $d$ de la source
reproduit son mouvement avec le \emph{retard} $t_P = d/c$, d'où
$y_P(t) = y_S(t - d/c)$. Le déphasage correspondant vaut
$\Delta\varphi = -\frac{2\pi d}{\lambda}$ ; réduire ensuite modulo
$2\pi$ pour l'interpréter (en phase, en opposition, en quadrature).
\end{methode}
