...
\begin{methode}
Changement d'inconnue : une équation en $e^x$ (ou un système en
$\ln x$) se ramène à une équation classique en posant $X = e^x$ (ou
$X = \ln x$). Résoudre en $X$, puis \emph{revenir à} $x$ en écartant
les valeurs impossibles — ici $X = -\frac{1}{3}$ est rejetée car
$e^x > 0$.
\end{methode}
  \end{sousq}
  \item \begin{sousq}
...
\begin{rappel}
Dérivée d'un quotient : $\left(\dfrac{u}{v}\right)' =
\dfrac{u'v - uv'}{v^2}$. Pour une fonction homographique
$\frac{ax+b}{cx+d}$, le résultat est toujours du type
$\frac{\text{constante}}{(cx+d)^2}$ : son signe est constant, la
fonction est monotone sur chaque intervalle de définition.
\end{rappel}
...
\begin{methode}
Ajustement par la méthode de Mayer : \textbf{1)} ordonner la série
selon $x$ ; \textbf{2)} la couper en deux moitiés et calculer les
points moyens $G_1$ et $G_2$ de chacune ; \textbf{3)} la droite
d'ajustement est $(G_1 G_2)$ — son équation s'obtient en résolvant le
petit système, ou par $a = \frac{y_{G_2} - y_{G_1}}{x_{G_2} - x_{G_1}}$.
\end{methode}
